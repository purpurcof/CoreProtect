...
public abstract class BaseInspector {
    ...
    protected void checkPreconditions(Player player) throws InspectionException {
        if (ConfigHandler.converterRunning) {
            throw new InspectionException(Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.UPGRADE_IN_PROGRESS));
        }

        if (ConfigHandler.purgeRunning) {
            throw new InspectionException(Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.PURGE_IN_PROGRESS));
        }
        ...
        if (ConfigHandler.lookupThrottle.get(player.getName()) != null) {
            ...
            if ((boolean) lookupThrottle[0] || (System.currentTimeMillis() - (long) lookupThrottle[1]) < 100) {
                throw new InspectionException(Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.DATABASE_BUSY));
            }
        }
    }
