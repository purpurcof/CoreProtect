    ...
    public static String performLookup(String command, Statement statement, BlockState block, CommandSender commandSender, int offset, int page, int limit) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (!found) {
                    resultTextBuilder = new StringBuilder(ChatUtils.getCoordinates(command, worldId, x, y, z, false, false) + "\n");
                }
                found = true;
            ...
            if (found) {
                ...
                if (rowMax > count && count > 0) {
                    resultText = Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.NO_RESULTS_PAGE, Selector.SECOND);
                }
                else {
                    // resultText = Phrase.build(Phrase.PREFIX) + Color.WHITE + "No block data found at " + Color.ITALIC + "x" + x + "/y" + y + "/z" + z + ".";
                    resultText = Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.NO_DATA_LOCATION, Selector.FIRST);
                    if (!blockName.equals("air") && !blockName.equals("cave_air")) {
                        resultText = Phrase.build(Phrase.PREFIX) + Phrase.build(Phrase.NO_DATA, Color.ITALIC + block.getType().name().toLowerCase(Locale.ROOT) + Color.WHITE) + "\n";
                    }
                }
            }
